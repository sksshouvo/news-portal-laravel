...
@section('general_body')
    ...
    @include('dashboard.partials.session')
        <h2>Designation Wise Permission</h2> 
        <div class="row">
         <div class="col-sm-12">
           <input type="button" name="add_new_desg_permission" id="add_new_desg_permission" class="btn btn-primary btn-block" value="New Designation Permission" data-toggle="modal" data-target="#new_desg_permission">
         </div>
        <br>
        <div class="col-sm-12">
          <br>
          ...
          @forelse ($desg_permission as $item)
          @php
          $desg_info = App\designation::where('id', '=',$item->desg_id)->first();
          ...
          @endphp
                <tr>
                  ...
                  <td style="text-align: center">
                      ...
                      <form action='desg_permissions/{{$item->id}}' method="POST" onclick="return confirm('Are you sure you want to delete this item?')">
                          @csrf 
                          @method('delete')
      ...
      {{$desg_permission->links()}}
    </div>
        
        
        </div>
        
 {{-- create new desg permission--}}
 <div class="modal fade" id="new_desg_permission" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <span class="modal-title">Create Designation Permission</span>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form method="POST" action="desg_permissions" id="create_permission">
            @csrf
            
          
          <label for="name">Designations: </label>
          <select name="desg_id" id="desg_id" class="form-control" required>
            <option value=""></option>
            @php
                $all_desg = App\designation::all();
            @endphp
            @forelse ($all_desg as $udata)
                <option value="{{$udata->id}}">{{$udata->designation}}</option>
            @empty
        ...
        <div class="modal-footer">
          <button type="submit" class="btn btn-primary" id="save_desg_permission" >Create Designation Permission</button>
