...
@section('general_body')
    ...
    <h2>Set News For cateory Page as Lead news </h2> 
    <h3>(User Can Add only two lead news for each category on Categories Page)</h3>
    <div class="row">
    <div class="col-sm-12">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <span class="modal-title">Set Lead News for category page</span>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="./set_as_cat" method="POST" id="create_set_as_cat" >
          @csrf
          @method('POST')
        ...
        <select name="category_id" id="category_id" class="form-control" required >
          <option value=""></option>
          @forelse ($all_cat as $ac)
        <option value="{{$ac->id}}">{{$ac->category}} ({{$ac->category_eng}})</option>
          @empty
